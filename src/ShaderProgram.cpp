...
bool ShaderProgram::loadShaders(const char* vsFilename, const char* fsFilename){
	...
	// po zlinkowaniu usuwamy shadery -> sa w programie shaderow
	glDeleteShader(vs);
	glDeleteShader(fs);
...
string ShaderProgram::fileToString(const string &filename){
	...
	try{
		...
		std::cerr << "ERROR: reading shader filen FAILED" << std::endl;
	}
...
void ShaderProgram::checkCompileErrors(GLuint shader, ShaderType type){
	// zmienne do sprawdzania statusu kompilowania shaderow
	int status = 0;
	...
	if ( type = PROGRAM ) {
		...
		if ( status = GL_FALSE ) {
			...
			std::cerr << "ERROR: Shader compile FAILED" << std::endl;
		}
	}
}
...
void ShaderProgram::setUniformSampler(const GLchar* name, const GLuint &slot) {
